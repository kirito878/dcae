...
class CleanResBlock(nn.Module):
    ...
    def forward(self, x, noise=None):  # 保留 noise 參數以維持介面一致
        x = self.depth_conv(x)
        x = self.ffn(x)
        return x
...
class NoiseInjectedResBlock(nn.Module):
    def __init__(self, ch, slope_depth_conv=0.01, inplace=False,
                 multiscale=(2, 4, 8), std_modulated=True):
        super().__init__()
        self.depth_conv = DepthConv(ch, ch, slope=slope_depth_conv, inplace=inplace)
        self.ffn = ConvFFN3(ch, inplace=inplace)
        ...
        self.std_modulated = std_modulated

    def _noise(self, ref, enable):
        B, _, H, W = ref.shape
        if not enable:
        ...
        for s in self.multiscale:
            ...
            small = torch.randn(B, 1, h, w, device=ref.device, dtype=ref.dtype)
            n = n + (small if s == 1 else
                     F.interpolate(small, size=(H, W), mode='bilinear',
                                   align_corners=False))
        return n / (len(self.multiscale) ** 0.5)

    def _inject(self, out, scale, enable):
        noise = self._noise(out, enable)
        if self.std_modulated:
            std = out.std(dim=[2, 3], keepdim=True) + 1e-5
            noise = noise * std
        return out + noise * scale

    def forward(self, x, enable_noise=True):     # 接口改:不再吃外部 noise
        out = self.depth_conv(x)
        out = self._inject(out, self.noise_scale1, enable_noise)   # 独立采样 #1
        out = self.ffn(out)
        out = self._inject(out, self.noise_scale2, enable_noise)   # 独立采样 #2
        return out
...
class NoiseTransform(nn.Module):
    def __init__(self, n_channels, mid_channels=None, no_noise=False):
        ...
        self.output_proj = nn.Conv2d(mid_channels, n_channels, 3, padding=1, bias=False)
        with torch.no_grad():
            self.output_proj.weight.normal_(mean=0.0, std=0.01)
        self.no_noise = no_noise

    def forward(self, cond):
        feat = self.input_proj(cond) if hasattr(self, 'input_proj') else cond
        enable = not self.no_noise
        for res in self.res_blocks_with_noise:
            # CleanResBlock 忽略 enable_noise 参数,NoiseInjectedResBlock 用它
            feat = res(feat, enable_noise=enable) if isinstance(res, NoiseInjectedResBlock) else res(feat)
...
class ResidualBottleneckBlockWithUpsample(nn.Module):
    ...
    def __init__(self, in_ch: int, out_ch: int, decoder_nt: bool = False):
        super().__init__()
...
class DCAE(CompressionModel):
    def __init__(self, head_dim=[8, 16, 32, 32, 16, 8], drop_path_rate=0, N=192,  M=320, num_slices=5, max_support_slices=5, **kwargs):
        ...
        self.m_up3 = [swin_block[0](feature_dim[0], feature_dim[0], self.head_dim[5], self.window_size, 0, basic_block[0], block_num=block_num[0], decoder_nt = False)] + \
                      [ResidualBottleneckBlockWithUpsample(feature_dim[0], output_image_channel,decoder_nt = True )]
