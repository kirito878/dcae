...
class NoiseInjectedResBlock(nn.Module):
    def __init__(self, ch, dilation=1):
        ...
        self.conv1 = nn.Conv2d(ch, ch, 3, padding=dilation,
                               dilation=dilation, padding_mode="reflect", bias=False)
    ...
    def forward(self, x, noise):
        out = self.conv1(x)
        std1 = out.std(dim=[2, 3], keepdim=True) + 1e-5
        out = out + noise * self.noise_scale1 * std1
        out = self.act(out)

        out = self.conv2(out)
        std2 = out.std(dim=[2, 3], keepdim=True) + 1e-5
        out = out + noise * self.noise_scale2 * std2
        out = self.act(out)
...
class CleanResBlock(nn.Module):
    """
    標準的殘差區塊 (後期使用，用來將帶噪聲的特徵平滑並轉換為空間紋理)
    """
...
class NoiseTransform(nn.Module):
    def __init__(self, n_channels, mid_channels=None, no_noise=False):
        super().__init__()
        mid_channels = mid_channels or n_channels * 2
        ...
        self.noise_proj = nn.Conv2d(n_channels, mid_channels, 1, bias=False)

        self.res_blocks_with_noise = nn.ModuleList([
            NoiseInjectedResBlock(mid_channels, dilation=1), 
            CleanResBlock(mid_channels, dilation=2),         
            NoiseInjectedResBlock(mid_channels, dilation=1), 
            CleanResBlock(mid_channels, dilation=2),        
            NoiseInjectedResBlock(mid_channels, dilation=1), 
            CleanResBlock(mid_channels, dilation=1),
        ])
    ...
    def forward(self, cond):
        ...
        for res in self.res_blocks_with_noise:
            raw_noise = self.gen_noise(feat)
            if self.no_noise:
                raw_noise = torch.zeros_like(raw_noise)
            feat = res(feat, raw_noise)
...
class SwinBlockWithConvMulti(nn.Module):
    def __init__(self, input_dim, output_dim, head_dim, window_size, drop_path, block=ResScaleConvolutionGateBlock, block_num=2, decoder_nt=False, **kwargs) -> None:
        ...
        if decoder_nt:
            self.noise_transform_1 = NoiseTransform(output_dim)
