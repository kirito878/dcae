...
class NoiseTransform(nn.Module):
    ...
    def forward(self, cond):
        ...
        for res in self.res_blocks_with_noise:
            ...
            if self.no_noise:
                raw_noise_1 = torch.zeros_like(raw_noise_1)
                raw_noise_2 = torch.zeros_like(raw_noise_2)
...
class DCAE(CompressionModel):
    ...
    def _collect_decoder_gate(self):
        ...
        for m in self.g_s.modules():
            if isinstance(m, NoiseInjectedResBlock) :
                if m.last_gate is not None:
                    found = m          # 不 return,繼續找,留最後一個
        return found.last_gate if found else None
