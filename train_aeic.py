...
class MultiFolderDataset(Dataset):
    """從多個資料夾遞迴蒐集影像,套用 transform 後回傳。"""
    IMG_EXT = ('.png', '.jpg', '.jpeg', '.bmp', '.webp')

    def __init__(self, roots, transform=None, exclude=None, min_size=None):
        ...
        for r in roots:
            for p in Path(r).rglob('*'):
                if p.suffix.lower() not in self.IMG_EXT:
                    continue
                if any(ex in p.parts for ex in exclude):
                    continue
                # 尺寸過濾：長或寬小於 min_size 就跳過
                if min_size is not None:
                    try:
                        w, h = Image.open(p).size       # 只讀 header,不解碼整張
        ...
        if len(self.samples) == 0:
            raise RuntimeError(f"在 {roots} 找不到任何影像")
        print(f"[MultiFolderDataset] 共載入 {len(self.samples)} 張影像 "
              f"(排除 {exclude}, 跳過過小 {skipped_small} 張, min_size={min_size})")
...
try:
    ...
    wd_fn = VGG16WassersteinDistortion().to(DEVICE)
...
def parse_args(argv):
    ...
    parser.add_argument("--exclude", type=str, nargs="*", default=[],
                        help="要排除的子資料夾名稱,例如 lsdir_10k(1024 階段用)")
    parser.add_argument("--min-size", type=int, default=None,
                            help="過濾掉長或寬小於此值的圖(1024 階段設 1024)")
